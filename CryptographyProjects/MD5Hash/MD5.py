...
class MyMD5:
    ...
    def __init__(self, A=0x67452301, B=0xEFCDAB89, C=0x98BADCFE, D=0x10325476, rotate_list=[0], constant_add=[0],
        ...
        self.C = C
        self.D = D
        self.debug = debug  # Зберігаємо стан дебагу

        self.r = rotate_list
        self.k = constant_add
        if len(rotate_list) != 64:
            self.r = [7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14,
                      20, 5, 9, 14, 20, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 6, 10, 15, 21, 6,
                      10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21]
        if len(constant_add) != 64:
            self.k = [0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
    ...
    def _print_state(self, label):
        if self.debug:
            print(f"[{label:^10}] A: {self.A:08x} | B: {self.B:08x} | C: {self.C:08x} | D: {self.D:08x}")
    ...
    def hash(self, data):
        self.reset()
        four_iter = [lambda x: (x % 16), lambda x: ((5 * x + 1) % 16), lambda x: ((3 * x + 5) % 16),
                     lambda x: ((7 * x) % 16)]

        if self.debug:
            print(f"ПОЧАТОК ХЕШУВАННЯ. Дані: {data}")


        for block_idx, words in enumerate(self.data_to_list(data)):
            if self.debug:
                print(f"\n--- Обробка блоку {block_idx} ---")
                words_hex = [f"{w:08x}" for w in words]
                print(f"Слова блоку: {words_hex}")

            AA, BB, CC, DD = self.A, self.B, self.C, self.D
            self._print_state("Init Block")
            ...
            for i in range(4):
                ...
                for j in range(16):
                    ...
                    if self.debug:
                        self._print_state(f"Step {general_count:02}")
            ...
            if self.debug:
                print("\n  Після додавання початкових значень блоку:")
                self._print_state("End Block")
...
if __name__ == "__main__":
    md5 = MyMD5(debug=True)
    result = md5.hash("a")
    print(f"\nФінальний хеш: {result}")
